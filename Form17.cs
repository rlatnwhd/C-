...
namespace C__Project
{
    public partial class Form17 : Form
    {
        ...
        private void CalculateButton_Click(object sender, EventArgs e)
        {
            ...
            if (!int.TryParse(Day1.Text, out workingDaysPerWeek) || workingDaysPerWeek <= 0)
            {
                MessageBox.Show("주의 근무일수를 올바르게 입력하세요.");
                return;
            }

            // DH1 텍스트 상자에서 주당 근무 시간을 입력받음
            if (!double.TryParse(DH1.Text, out weeklyHours) || weeklyHours <= 0)
            {
                MessageBox.Show("주당 근무 시간을 올바르게 입력하세요.");
                return;
            }
            ...
            // 시급 계산
            double hourlySalary = monthlySalary / (workingDaysPerWeek * weeklyHours);
